...
do{
	...
	foreach($article_array as $article){
		...
		$aid=$board->addArticle(
				$category,$article['title'],$article['data'],
				$article['writer']['s_name'],$article['writer']['n_id'],
				0,0,array());
		if($aid===false){
		...
		foreach($article['comments'] as $comment){
			...
			$cid=$board->addArticle(
				$category,mb_substr($comment['data'],0,15), $comment['data'],
				$comment['writer']['s_name'],$comment['writer']['n_id'],
				$aid, 0, array());
