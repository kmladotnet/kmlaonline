...
$_LANGUAGE=array(
	...
	"user"=>array(
		...
		"register"=>array(
			"title"=>"회원가입",
			// "tos"=>"현재 작업 중입니다.",
			"tos"=>"KMLA Online은 민족사관고등학교 동문 간 편리한 소통, 그 외 신원 확인을 목적으로 최소한의 개인정보를 수집합니다. 이는 E-Mail, 이름, 생일 등을 포함합니다. <br /> *** <br />학번은 선택 사항이지만 추후 본인과 타인의 쉽고 원활한 소통을 위하여 입력하시는 것을 권장드립니다. <br />***",
	...
	"generic"=>array(
		...
		"picture"=>"사진",
		"icon"=>"아이콘",
		"status message"=>"자기소개 메시지",
		"homepage"=>"SNS",
		"phone"=>"전화번호",
		"interest"=>"관심사/전공",
        "suggest"=>"민사고 동문임을 확인하기 위해 필요한 정보입니다.",
		"gender"=>"성별",
		"unspecified"=>"지정하지 않음",
		"male"=>"남자",
		"female"=>"여자",
	...
	"layout"=>array(
		...
		"footer"=>'<div style="display: table;/* width: 100%; */margin: auto;width: calc(58% + 320px);"><div style="display: table-cell;text-align: left;">닷넷 2007 - 2017</div><div style="display: table-cell;text-align: right;"><img src="/images/kmlaonline-p-resize.png" style="/* display: block; *//* margin: auto; */height: 24px;display: inline-block;/* float: right; */"></div></div>'
	),
    "weather"=>array(
        200 => '뇌우',
        201 => '뇌우',
...
function elang($key1,$key2,$key3=null,$key4=null){ echo lang($key1,$key2,$key3,$key4); }
function elangraw($key1,$key2,$key3=null,$key4=null){ echo langraw($key1,$key2,$key3,$key4); }
...
function langraw($key1,$key2,$key3=null,$key4=null){
	global $_LANGUAGE;
	if(is_null($key3)) return isset($_LANGUAGE[$key1][$key2])?$_LANGUAGE[$key1][$key2]:"$key1 $key2";
	if(is_null($key4)) return isset($_LANGUAGE[$key1][$key2][$key3])?$_LANGUAGE[$key1][$key2][$key3]:"$key1 $key2 $key3";
	return isset($_LANGUAGE[$key1][$key2][$key3][$key4])?$_LANGUAGE[$key1][$key2][$key3][$key4]:"$key1 $key2 $key3 $key4";
}
