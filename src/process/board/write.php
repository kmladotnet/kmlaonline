	...
	if($article_id===false){
		...
		if(preg_match_all('/@(((찾기)\\{[^}]*\\}|[^\\s`~!@#$%^&*\\(\\)\\-=_+\\[\\]\\\\{}\\|;\':\",.\\/\\<\\>\\?]*))/i',strip_tags($s_data),$tagged)>0){
			...
			foreach($triggered as $usr){
				if($article_flag & 0x4){	 // Anonymous
					...
					addNotification(1, $usr['n_id'], "tagged:article:$article_id", "익명 사용자가 {$_POST['s_title']}으로 태그하였습니다.","article:".$article_id);
				}else{
					addNotification($me['n_id'], $usr['n_id'], "tagged:article:$article_id", "{$me['s_name']}님이 {$_POST['s_title']}으로 태그하였습니다.","article:".$article_id);
				}
			}
		}
		...
		if((int) $cat['n_id'] === 576){
			...
			foreach(listDotnet() as $usr){
				addNotification($me['n_id'], $usr['n_id'], "upload:article:$article_id", "{$me['s_name']}님이 닷넷 <b>HTML/CSS</b> 그룹에 게시글을 올렸습니다.","article:".$article_id);
			}
		}

		/* 전공 게시판에 글 올리면 notification 뜨게 하기*/
		if((int) $cat['n_id'] === 2 && isset($_POST['s_title']) && !isset($_POST['n_parent'])){
			foreach($member->listMembers(0,0,19) as $usr){
				addNotification($me['n_id'], $usr['n_id'], "upload:article:$article_id", "<b>" . putUserCard($me,0,false) . "</b>님이 <b>전체 공지사항</b> 그룹에 게시글을 올렸습니다.", "article:".$article_id);
			}
		...
		if(isset($_POST['n_parent'])){
			...
			if($article_flag & 0x4) { // Anonymous
				$tagby=lang("terms","anonymous");
				addNotification(1, $parent_writer['n_id'], "reply:article:$article_id:from:".$me['n_id'], "익명 사용자가 <b>".htmlspecialchars($parent_article['s_title'])."</b>에 답변을 달았습니다.","/board/{$cat['s_id']}/view/$article_id");
				}
			else{
				addNotification($me['n_id'], $parent_writer['n_id'], "reply:article:$article_id:from:".$me['n_id'], putUserCard($me,0,false)."님이 <b>".htmlspecialchars($parent_article['s_title'])."</b>에 답변을 달았습니다.","/board/{$cat['s_id']}/view/$article_id");
			}
		}
		if(checkCategoryAccess($cat['n_id'], "attach upload")){
