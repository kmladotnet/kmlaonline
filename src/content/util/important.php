		...
		<tbody>
			<?php while ($row = $res->fetch_array(MYSQLI_ASSOC)){
				...
				<tr style="height:32px;border-top:1px solid gray;" class="real-item">
					...
					<td style="cursor:pointer" onclick="changeLinkTo('/board/<?php echo $cat['s_id']?>/view/<?php echo $article['n_id']?>');"><a href="/board/<?php echo $cat['s_id']?>/view/<?php echo $article['n_id']?>"><?php echo htmlspecialchars($article['s_title']); ?></a></td>
