...
<?php putUserCard($usr); ?></a></td>
						...
						<td>신청자가 없습니다</td>
						<td>
						<?php $date1 = new DateTime("now"); $date2 = new DateTime("2025-02-17 00:00:00"); // update DateTime constructor parameter
						if($date1 >=  $date2) { ?>
...
<?php putUserCard($usr); ?></a></td>
					...
					<td>
					<?php $date1 = new DateTime("now"); $date2 = new DateTime("2025-02-17 00:00:00");
					if($date1 >= $date2) { ?>
						<form method="post" action="/proc/util/donation" onsubmit="return saveAjax(this,'신청중...');">
							<input type="hidden" name="from" value="book">
							<input type="hidden" name="category" value="<?php echo $category ?>" />
