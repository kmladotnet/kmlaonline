			...
			<tbody>
				<?php for($num = 1; $num <= sizeof($currentTable[$category]); $num++){ ?>
					<tr>
						...
						<td>신청자가 없습니다</td>
						<td>
						<?php $date1 = new DateTime("now"); $date2 = new DateTime("2018-02-06"); // update DateTime constructor parameter
						if($date1 >=  $date2) { ?>
		...
		<tbody style="text-align: center;">
			<?php for($num = 1; $num <= sizeof($currentTable[$category]); $num++){ ?>
				<tr>
					...
					<td>
					<?php $date1 = new DateTime("now"); $date2 = new DateTime("2018-02-06");
					if($date1 >= $date2) { ?>
						<form method="post" action="/proc/util/donation" onsubmit="return saveAjax(this,'신청중...');">
							<input type="hidden" name="from" value="book">
							<input type="hidden" name="category" value="<?php echo $category ?>" />
