						...
						<tr>
							...
							<td>
								<form method="post" action="/check" onsubmit="return saveAjax(this,'카테고리 추가 중...');">
									...
									<div style="display:inline-block;width:100px;">카테고리 종류:</div><select name="new_cat_type" style="width:100px;">
									...
									?>
											<div id="class_group_<?php echo $i / $cpp ?>" style="overflow:hidden;<?php if ($i != 0) echo "display:none;"; ?>">
												<table style="width:100%;" class="table_info_view">
													<tr>
														<th style="width:24px;text-align:left;">수정</th>
														<th style="width:24px;text-align:left;">번호</th>
														<th style="width:80px;text-align:left;">카테고리 ID</th>
														<th style="text-align:left;">카테고리 이름</th>
														<th style="width:48px;text-align:left;">글 수</th>
													</tr>
												<?php
											}
											if ($val['n_id'] == 1) {
...
<?php echo htmlspecialchars($val['s_desc']) ?></textarea>
													...
													<tr class="cat_edit_form_<?php echo $val['n_id'] ?>" style="height:16px;display:none">
														<td colspan='5'></td>
													</tr>
											<?php
											}
						...
						<table class="table_info_view">
							<tr>
								...
								<td>
									<?php $sel = "";
									$a = false;
									foreach ($possible_permissions as $k => $v) { ?>
										<input onclick="fetchSpecialPermissioners('<?php echo $k ?>');" type="radio" name="permission_type" id="permission_type_<?php echo $k ?>" value="<?php echo $k ?>" <?php if (!$a) {
																																																			$a = true;
