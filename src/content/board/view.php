		...
		<?php foreach($board->getArticleList(false, false, $parent, 0, 0, "n_id", false, 0, false, true, true, false, false, false, false, true) as $comment){ $m=$member->getMember($comment['n_writer']);?>
			<div id="article_comment_sub_<?php echo $comment['n_id']?>" class="acomment">
				...
				if($m['s_pic'] && !$b_comment_anonymous)
                    ...
                    echo '<div class="circle anonymous-bubble" style="width:'.$pic_sz.'px; height:'.$pic_sz.'px; background:rgb('.hsvToRgb($hue,50,50).');">'.substr(base_convert($hash_val, 16, 36),2,1).'</div>';
				}
				?>
			...
			<div style="margin-left:12px"><?php putCommentTree($comment['n_id'],$root); ?></div>
		<?php } ?>
	</div><?php
