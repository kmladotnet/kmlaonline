...
function printVotes($id, $hide = true)
{
    ...
    ?>">
        <button type="button" data-id="<?php echo $id; ?>" id="unvoted-<?php echo $id; ?>" class="btn btn-default"
            style="color: black; padding: 0 4px; height: 24px;<?php if ($upvoted) {
        echo 'display: none;';
    }
    ?>"
...
<?php $uv = upvoters($id);if (empty($uv)) {
        echo '-';
    } else {
        foreach ($uv as $p => $v) {
...
function printUpDownVotes($id, $hide = true)
{
    ...
    ?>" style="font-weight: bold; color: forestgreen; padding: 0px; height: 24px; width: 24px;"
    ...
    ?>" data-tooltip="big-tooltip" trigger="hover" title="&quot;동의하지 않음&quot; 버튼이 아닙니다." style="font-weight: bold; color: crimson; padding: 0px; height: 24px; width: 24px;"
...
<?php $dv = downvoters($id);if (empty($dv)) {
        echo '-';
    } else {
        foreach ($dv as $p => $v) {
            echo $member->getMember($p)['s_name'] . ' ';
...
<?php
$path = "https://{$_SERVER['SERVER_NAME']}/files/bbs/{$cat['n_id']}/{$article['n_id']}/{$v['n_id']}/{$v['s_key']}/" . rawurlencode($v['s_name']);
            $path_thumb = "https://{$_SERVER['SERVER_NAME']}/files/bbs/{$cat['n_id']}/{$article['n_id']}/{$v['n_id']}/{$v['s_key']}/$thumbmode/" . rawurlencode($v['s_name']);
            $path_force = "https://{$_SERVER['SERVER_NAME']}/files/bbs/{$cat['n_id']}/{$article['n_id']}/{$v['n_id']}/{$v['s_key']}/force/" . rawurlencode($v['s_name']);
            $file_types = array(
...
function printViewPage()
{
    ...
    $mode = $cat['n_viewmode'];
    switch ($mode) {
        case 0:printViewPageModeBoard($usr, $cat);
            break;
        case 1:printViewPageModeGallery($usr, $cat);
            break;
        case 2:printViewPageModeForum($usr, $cat);
            break;
    }
    /*
$search=checkCategoryAccess($board_cat['n_id'], "search") && ((isset($_GET['search']) && $_GET['search']!="")?$_GET['search']:false);
