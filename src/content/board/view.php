...
function printVotes($id, $hide = true)
{
    ...
    if ($hide) { ?>
        <button type="button" data-id="<?php echo $id; ?>" id="collapse-<?php echo $id; ?>" class="btn btn-default" data-tooltip="tooltip" trigger="hover" title="글 숨기기" style="color: royalblue; padding: 0px; height: 24px; width: 24px; float: left; margin-right: 4px" onclick='hidePost($(this).data("id"));'>
            <i class="fa fa-minus"></i>
        </button>
        <button type="button" data-id="<?php echo $id; ?>" id="uncollapse-<?php echo $id; ?>" class="btn btn-default" data-tooltip="tooltip" trigger="hover" title="글 보이기" style="color: royalblue; padding: 0px; height: 24px; width: 24px; float: left; display: none; margin-right: 4px" onclick='showPost($(this).data("id"));'>
            <i class="fa fa-plus"></i>
        </button>
    <?php } ?>
    <div style="display: inline-block; vertical-align: middle; <?php if (getTheme($me)['voteright']) {
                                                                    echo 'float:right;';
                                                                }
                                                                ?>">
        <button type="button" data-id="<?php echo $id; ?>" id="unvoted-<?php echo $id; ?>" class="btn btn-default" style="color: black; padding: 0 4px; height: 24px;<?php if ($upvoted) {
                                                                                                                                                                            echo 'display: none;';
                                                                                                                                                                        }
                                                                                                                                                                        ?>" onclick='upvote($(this).data("id"), false);'>
            <?php echo '&#43;' . $upvotes; ?>
        </button>
        <button type="button" data-id="<?php echo $id; ?>" id="upvoted-<?php echo $id; ?>" class="btn btn-default" style="font-weight: bold; color: forestgreen; padding: 0 4px; height: 24px;<?php if (!$upvoted) {
                                                                                                                                                                                                    echo 'display: none;';
                                                                                                                                                                                                }
    ...
    <span id='likers-<?php echo $id; ?>' style="display:none; vertical-align: top; font-size: 0.9em;">
        <div style='color: forestgreen'> <strong>좋아요:</strong>
            ...
            if (empty($uv)) {
                ...
                foreach ($uv as $p => $v) {
                    echo $member->getMember($p)['s_name'] . ' ';
                }
            }
...
function printUpDownVotes($id, $hide = true)
{
    ...
    <?php } ?>
    <div class="input-group" style="display: inline-table; vertical-align: middle; width: 1px;<?php if (getTheme($me)['voteright']) {
                                                                                                    echo 'float:right;';
                                                                                                }
                                                                                                ?>">
        <span class="input-group-btn">
            <button type="button" data-id="<?php echo $id; ?>" id="plus-<?php echo $id; ?>" data-toggle="button" class="btn btn-default
                ...
                ?>" style="font-weight: bold; color: forestgreen; padding: 0px; height: 24px; width: 24px;" onclick='if($(this).hasClass("active")) unvote($(this).data("id"), true); else upvote($(this).data("id"), true);'>
                <i class="fa fa-arrow-up"></i>
            </button>
        </span>
        <span id="downvote-<?php echo $id; ?>" class="input-group-btn" style="<?php if (!$downvoted) {
                                                                                    echo 'display:none';
                                                                                }
                                                                                ?>">
        ...
        <span id="vote-<?php echo $id; ?>" class="input-group-btn" style="<?php if ($upvoted || $downvoted) {
                                                                                echo 'display:none';
                                                                            }
                                                                            ?>">
        ...
        <span id="upvote-<?php echo $id; ?>" class="input-group-btn" style="<?php if (!$upvoted) {
                                                                                echo 'display:none';
                                                                            }
                                                                            ?>">
        ...
        <span class="input-group-btn">
            <button type="button" data-id="<?php echo $id; ?>" id="minus-<?php echo $id; ?>" data-toggle="button" class="btn btn-default
                ...
                ?>" data-tooltip="big-tooltip" trigger="hover" title="&quot;동의하지 않음&quot; 버튼이 아닙니다." style="font-weight: bold; color: crimson; padding: 0px; height: 24px; width: 24px;" onclick='if($(this).hasClass("active")) unvote($(this).data("id"), true); else downvote($(this).data("id"));'>
...
function printAttachList($article, $cat, $mode = 0)
{
    ...
    if (!checkCategoryAccess($cat['n_id'], "attach download")) {
        return false;
    }
        ...
        <div style="border:1px solid #DDD;margin:5px;padding:5px;">
            <div id="article_comment_sub_<?php echo $article['n_id'] ?>" class="acomment">
                <div>
                    <?php if ($b_anonymous) {
                        echo "익명";
                    } else { ?><a href="<?php echo "/user/view/{$m['n_id']}/" . htmlspecialchars($m['s_id']) ?>">
                            <?php putUserCard($m) ?></a>
                    <?php } ?></div>
...
?><div id="article_comment_<?php echo $parent ?>" class="article_comment">
        <?php foreach ($board->getArticleList(false, false, $parent, 0, 0, "n_id", false, 0, false, true, true, false, false, false, false, true) as $comment) {
            $m = $member->getMember($comment['n_writer']); ?>
        ...
        <?php if ($is_mobile) { ?>
            ...
            <div class="item_contents" style="padding:10px;padding-bottom:7px;overflow:auto">
                ...
                <div class="forum_comment_area">
                    <?php if ($article['n_comments'] > 0 && !$suppress_comments) {
                        putCommentTree($article['n_id'], $article['n_id']);
                    } ?>
                </div>
        ...
        <?php } else { ?>
            ...
            <div class="item_contents" style="padding:10px;padding-bottom:7px;overflow:auto">
                ...
                <div class="new_forum_comment_area">
                    <?php if ($article['n_comments'] > 0 && !$suppress_comments) {
                        putCommentTree($article['n_id'], $root);
                    } ?>
                </div>
                <div id="article_comment_<?php echo $article['n_id'] ?>"></div>
                <div style="clear:both"></div>
...
function printViewPageModeForum($usr, $cat)
{
    ...
    $pagenumber++;
    $k = $_GET;
    unset($k['action'], $k['bid'], $k['bact'], $k['page'], $k['bitm'], $k['bcmt']);
...
?>
    ...
    <div style="text-align:center">
        ...
        <a href="<?php echo "/board/$board_id/view/{$article['n_id']}/1$additional_query_string"; ?>" <?php if ($pagenumber == 1) {
        ...
        <?php for ($i = max(2, $pagenumber - 10); $i <= min($page_count - 1, $pagenumber + 10); $i++) { ?>
            <a href="<?php echo "/board/$board_id/view/{$article['n_id']}/$i$additional_query_string"; ?>" <?php if ($pagenumber == $i) {
                                                                                                                echo "style='color:black'";
                                                                                                            }
                                                                                                            ?>>[<?php echo $i;
                                                                                                                $disp[$i] = true; ?>]</a>
...
function printViewPage()
{
    ...
    switch ($mode) {
        case 0:
            printViewPageModeBoard($usr, $cat);
            break;
        case 1:
            printViewPageModeGallery($usr, $cat);
            break;
        case 2:
            printViewPageModeForum($usr, $cat);
            break;
    }
    /*
$search=checkCategoryAccess($board_cat['n_id'], "search") && ((isset($_GET['search']) && $_GET['search']!="")?$_GET['search']:false);
