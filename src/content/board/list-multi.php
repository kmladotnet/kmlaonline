...
function printArticleList(){
    ...
    $multi_id = arrayToCategories($board_id["cat"]);
	$articleperpage=20;
	$orderby_name="n_id";
	$orderby_desc=true;
	$incl_text=0;
	...
	$article_count=$board->getArticleCount($multi_id, false, 0, $search, $search_mode_and, $search_submode_and, $search_title, $search_data, $search_tag, $search_writer);
	...
	$board_data=$board->getArticleList($multi_id, false, 0, $pagenumber, $articleperpage, $orderby_name, $orderby_desc, $incl_text, $search, $search_mode_and, $search_submode_and, $search_title, $search_data, $search_tag, $search_writer);
	if($search!==false){
	...
	<div style="text-align:center">
        <ul class="pagination">
            ...
            <li <?php if($pagenumber==1) echo "class='active'";?>>
                <a href="<?php echo "/board/",urlencode(json_encode($board_id)),"/page/1$additional_query_string" ;?>">1</a>
            </li>
            ...
            for($i=max(2,$pagenumber-$max_show); $i<=min($page_count-1, $pagenumber+$max_show); $i++) { ?>
                <li <?php if($pagenumber==$i) echo "class='active'" ?>>
                    <a href="<?php echo "/board/",urlencode(json_encode($board_id)),"/page/$i$additional_query_string" ;?>">
            ...
            if(!isset($disp[$page_count]) && $page_count>1) { ?>
                <li <?php if($pagenumber==$page_count) echo "class='active'" ?>>
                    <a href="<?php echo "/board/",urlencode(json_encode($board_id)),"/page/$page_count$additional_query_string" ;?>">
                        <?php echo $page_count?>
                    </a>
                </li>
            <?php } ?>
	...
	<div style="float:right;position:relative;">
		<form method="get" action="/board/<?php echo htmlspecialchars(urlencode(json_encode($board_id)))?>">
			...
			<input type="text" class="form-control" name="search" value="<?php echo $search?htmlspecialchars($search):""?>" style="margin-left:3px;width:120px;vertical-align:middle;" />
