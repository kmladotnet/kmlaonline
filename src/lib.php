...
require __DIR__ . "../config/db-config.php"; /* LOAD DATABASE - DO NOT REMOVE */
...
function putUserCard($m, $mode = 0, $putNow = true)
{
    ...
    if ($m['n_level'] > 0 && $m['n_level'] < 10000) {
        switch ($mode) {
            case 0:$str .= $m['n_level'] . "기 " . htmlspecialchars($m['s_name']);
                break;
            case 1:$str .= htmlspecialchars($m['s_name']) . " (" . $m['n_level'] . "기)";
                break;
        }
    } else {
...
function getLinkDescription($lnk)
{
    ...
    $fnc = substr($lnk, 1);if (strpos($fnc, "/") !== false) {
        $fnc = substr($fnc, 0, strpos($fnc, "/"));
    }

    $sub = substr($lnk, strlen($fnc) + 2);if (strpos($sub, "/") !== false) {
