...
class ZipStream
{
    ...
    private $addExtraField = false;

    private $streamChunkSize = 262144;
    ...
    public function addFile($data, $filePath, $timestamp = 0, $fileComment = null)
    {
        ...
        $gpFlags = "\x00\x08"; // General Purpose bit flags for compression type 8 it is: 0=Normal, 1=Maximum, 2=Fast, 3=super fast compression.
        ...
        if ($gzLength >= $dataLength) {
            ...
            $gpFlags = "\x00\x08"; // Compression type 0 = stored
        }
    ...
    private function processFile($dataFile, $filePath, $timestamp = 0, $fileComment = null)
    {
        ...
        $dataLength = $this->_filesize($dataFile);
        if ($dataLength >= 0) {
            $gzType = "\x00\x00"; // Compression type 0 = stored
            $gpFlags = "\x00\x08"; // Compression type 0 = stored
            $fn_mine = str_replace("\\", "/", $dataFile);
            $key = sha1($fn_mine);
            $cache_file_dir = "data/cache/" . substr($key, 0, 2) . "/" . substr($key, 2, 2) . "/" . substr($key, 4, 2) . "/";
            @mkdir($cache_file_dir, 0777, true);
            $cache_file_path = $cache_file_dir . substr($key, 6) . "-" . filemtime($fn_mine);
            $fileCRC32 = "";
            if (file_exists($cache_file_path)) {
                $fileCRC32 = @file_get_contents($cache_file_path);
            } else {
                $fileCRC32 = strrev(hash_file("crc32b", $dataFile, true));
                @file_put_contents($cache_file_path, $fileCRC32);
            }
            if ($fileCRC32 == "") {
                $fileCRC32 = strrev(hash_file("crc32b", $dataFile, true));
            }

            $this->buildZipEntry($filePath, $fileComment, $gpFlags, $gzType, $timestamp, $fileCRC32, $dataLength, $dataLength, self::EXT_FILE_ATTR_FILE);

            $file_handle = fopen($dataFile, "rb");
            while (!feof($file_handle)) {
                echo fread($file_handle, $this->streamChunkSize);
            }
            fclose($file_handle);
        } else {
