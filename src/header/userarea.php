...
<?php } else { ?>
		...
		<tr style="height:100px">
			...
			<td style="text-align:center" id="profile_nav">
				<form method="post" action="/user/logout" id="logout_form" onsubmit="return true;">
					...
					<div>
						<?php putUserCard($me); // lib.php 
							?></div>
					<div><a href="<?php echo "/user/view/{$me['n_id']}/{$me['s_id']}" ?>">
							<?php echo lang("generic", "information"); ?></a> | <a href="/user/message">
							<?php echo lang("generic", "message"); ?></a></div>
					<div><a href="/user/settings">
							<?php echo lang("generic", "setting"); ?></a> | <a href="/user/manage">
							<?php echo lang("generic", "manage"); ?></a></div>
					<div><a href="/board/special:list-all">모든글</a>
						<?php if ($me['n_admin'] > 0) { ?> | <a href="/admin">
								<?php echo lang("generic", "admin"); ?></a>
						<?php } ?></div>
					<a href="/user/logout" onclick="$('#logout_form').submit(); return false;">
	...
	<div style="overflow:hidden;width:222px;display:block;margin-top:-6px;">
		<a href="#" onclick="return editStatusMessageShow();">
			<div id="status_message">
				<?php echo htmlspecialchars($me['s_status_message']) ?></div>
		</a>
