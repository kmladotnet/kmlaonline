<div>
	<?php if ($is_mobile) { ?>
		<div id="total-header">
			<div class="userarea">
				...
				<?php if (isset($_SESSION['user'])) {  ?>
					<a href="<?php echo "/user/view/{$me['n_id']}/{$me['s_id']}" ?>" class="userinfo">
						<?php
								if ($me['s_pic']) {
									?><img src="<?php echo htmlspecialchars($me['s_pic']) ?>" style="padding-right:2px" />
						<?php
								} else {
									?>
							<img src="/images/no-profile.png" style="padding-right:2px" />
						<?php
								}
								?><span style="line-height:48px;">
							<?php echo htmlspecialchars($me['s_name']); ?>
						</span>
						<?php
								?>
					</a>
		...
		<?php } else if ($_SERVER["REQUEST_URI"] !== '/util/library' && $_SERVER["REQUEST_URI"] !== '/util/chat') {
			...
			<div id="total-header" style="background: url('/images/check-penalty-of-council.png') no-repeat; background-size: contain;" <?php if ($april_fools_2) echo 'style="height:311px; background: url(/images/bamboozle.png) no-repeat;"'; ?>>
				<?php if (isset($_SESSION['user'])) { ?>
					<div class="userarea">
						<?php include "src/header/userarea.php"; ?></div>
				<?php } ?>
				<a id="back-to-main" href='https://bit.ly/2n5ArDt'></a><br />
			</div>
	<?php }
	} ?>
	...
	<div id="total-content">
		<?php if (function_exists("printContent")) {
			printContent();
		} ?>
