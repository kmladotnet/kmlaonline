<div>
	...
	<?php } else {
        if(!(!!$me && getTheme($me)['hidedasan'])) { ?>
            <div id="total-header" style="background: url('/images/big-logo-ad-lid.png') no-repeat;" <?php if($april_fools_2) echo 'style="height:311px; background: url(/images/bamboozle.png) no-repeat;"'; ?>>
                <?php if (isset($_SESSION['user'])) { ?>
                    <div class="userarea"><?php include "src/header/userarea.php"; ?></div>
                <?php } ?>
                <a id="back-to-main" href="board/all_announce/view/470396"></a><br />
            </div>
        <?php } ?>
