...
function search_split_terms($terms){
		...
		$terms = preg_replace("/\"(.*?)\"", function($1) {return search_transform_term($1)}, $terms);
		...
		foreach($terms as $term){
			...
			$term = preg_replace("/\{WHITESPACE-([0-9]+)\}", function($1) {return chr($1);}, $term);
	...
	function search_transform_term($term){
		$term = preg_replace("/(\s)", function($1) { return '{WHITESPACE-'.ord('\$1').'}';}, $term);
		$term = preg_replace("/,/", "{COMMA}", $term);
		return $term;
	}
