...
if(!function_exists("__Soreeboard__removeSpaces")){
	function __Soreeboard__removeSpaces($k){ return str_replace(" ","_",$k); }
}
class Soreeboard{
	...
	function setCategoryPermission($cat, $action, $permission){
		...
		$this->last_error=$this->mysqli->error;$this->last_errno=$this->mysqli->errno;
		return false;
	}
	...
	function isUserAllowed($cat, $member, $action, $consider_admin=true){
		...
		if(!is_numeric($action=$this->getCategoryActionInt($action))) return false;
		...
		if($consider_admin) if($member['n_admin']!=0 && $perm==false) $perm=-1;
		return $perm;
	}
	...
	function addArticle($category,$title,$data,$writer_s,$writer_n=1,$parent=0,$sticky=0,$attach=array(),$tag=""){
		...
		foreach($attach as $value){ if(!is_numeric($value)) return false; }
		...
		if($parent>0) $category=$parent_article['n_cat']; else $parent="null";
	...
	*/
	function setViewFlag($id, $key=false, $member=false){
		...
		$this->mysqli->autocommit(false);
		if($member==false) $memberq="null"; else $memberq=$member;
