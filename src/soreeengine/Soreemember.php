...
class Soreemember
{
	...
	function getMember($member, $by = 0, $withpw = false)
	{
		...
		switch ($by) {
			case 0:
				if (isset($this->member_cache["n_id:" . $member])) {
					return $this->member_cache["n_id:" . $member];
				}
				$query .= "n_id=$member";
				break;
			case 1:
				if (isset($this->member_cache["s_id:" . $member])) {
					return $this->member_cache["s_id:" . $member];
				}
				$query .= "s_id='$member'";
				break;
			case 2:
				if (isset($this->member_cache["s_email:" . $member])) {
					return $this->member_cache["s_email:" . $member];
				}
				$query .= "s_email='$member'";
				break;
		}
		if ($res = $this->mysqli->query($query)) {
			while ($row = $res->fetch_array(MYSQLI_ASSOC)) {
	...
	function sendNote($member_from, $member_to, $data, $title = "")
	{
		...
		if ($member_from != $member_to) {
			if (
				($this->mysqli->query("INSERT INTO `$this->table_note` (n_owner, n_from, n_to, n_date, s_title, s_data) VALUES ($member_from, $member_from, $member_to, $date, '$title', '$data')") === true) && ($this->mysqli->query("INSERT INTO `$this->table_note` (n_owner, n_from, n_to, n_date, s_title, s_data) VALUES ($member_to, $member_from, $member_to, $date, '$title', '$data')") === true)
			)
				return $this->mysqli->insert_id;
			else
				return false;
		} else {
			return ($this->mysqli->query("INSERT INTO `$this->table_note` (n_owner, n_from, n_to, n_date, s_title, s_data) VALUES ($member_from, $member_from, $member_to, $date, '$title', '$data')") === true);
		}
	}
